...
export class GeoHash{
    ...
    private decodeGeohash():GeoCoordinate[]{
        ...
        const lat: number[] = []; const lon: number[] = [];
        lat[0] = -90.0;  lat[1] = 90.0;
        lon[0] = -180.0; lon[1] = 180.0;
        let latError: number = 90.0; let lonError: number = 180.0;
        
        // I get it, linter, this is bad form. But I'm itterating over chars in a string and 
        //  don't want to refactor with some string.split() BS 
        // tslint:disable-next-line:prefer-for-of
        for (let i: number = 0; i<this.geohash.length; i++) {
            const c:string = this.geohash[i];
            const cd = this.BASE32.indexOf(c);
            for (let j:number=0; j<5; j++) {
                const mask:number = this.BITS[j];
                if (isEven) {
                    lonError /= 2;
    ...
    private countHashesUntilDistance(distance: number, direction: 'n'|'s'|'e'|'w', points: number=1):number
    {
        ...
        while(pointsInScope >= points){
            ...
            const currentBlock = new GeoHash(hash)
    ...
    getAllHashStringsWithinRadius(distance: number, points: 1|2|3|4|5 = 1): string[]{
        ...
        for (let southCounter = 0; southCounter < searchSouth; southCounter++){
            hashes.push(new GeoHash(currentRow))
        ...
        // and filter
        const hashesInRange = hashes.filter(o => o.getAllCoordinates().map(p=>p.distanceTo(this.centroid)).filter(q => q <= distance).length >= points)
